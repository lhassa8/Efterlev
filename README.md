...
Also ask me for the absolute path to my **repo root** (not the Terraform
subdirectory if the repo is mixed-content). Efterlev walks subdirectories
for `.tf` files, reads `.github/workflows/*.yml` for the github-source
detectors, and loads `.efterlev/manifests/*.yml` for procedural KSI
attestations — all three only fire when scanned from the repo root. If
I name a Terraform-only subdir like `infra/terraform/`, the workflow
detectors and manifests get silently skipped and ~20% of detector
coverage disappears.
