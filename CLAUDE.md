...
- 395 tests passing (+ 1 E2E skipped by default without `ANTHROPIC_API_KEY`).
...
- **Secret redaction before LLM transmission is implemented and unconditional**: 7 pattern families, fail-closed, audit-ledger-optional, 0 false-positives on real govnotes infrastructure.
...
- Redaction ledger-to-disk + `efterlev redaction review` CLI (small audit-sugar follow-up to secret redaction).
