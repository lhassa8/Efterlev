...
- 381 tests passing (+ 1 E2E skipped by default without `ANTHROPIC_API_KEY`).
- ruff clean; mypy strict-clean on 95 source files (strict on `efterlev.{primitives,detectors,oscal,manifests}.*`).
...
- **Secret redaction before LLM transmission is implemented and unconditional**: 7 pattern families, fail-closed, audit-ledger-optional, 0 false-positives on real govnotes infrastructure. THREAT_MODEL.md describes the actual pass, not a planned one.

**What's next (per v1 locked plan + 2026-04-23 review deferred items):**
- Retry + Opus-to-Sonnet fallback on transient errors (brings back `fallback_model` config field). Previously ranked as highest-leverage v1.x; now the next-highest after secret redaction landed.
- Redaction ledger-to-disk + `efterlev redaction review` CLI (small audit-sugar follow-up).
