...
- **Path-argument confused-deputy bound.** MCP tools accept `target` paths from the client. The server requires `.efterlev/` to exist under the target and reads `.tf` files relative to that root, so a hostile client can only touch repos that already have an Efterlev workspace structure. The same client has shell-equivalent access (they spawned the subprocess) — the path argument doesn't expand their capabilities, just their convenience. Worth naming explicitly in the threat model so future refactors don't accidentally widen the path-argument's reach (e.g., supporting absolute paths to arbitrary directories without the workspace-structure check).
