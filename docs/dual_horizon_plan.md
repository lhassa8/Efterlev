...
- **Phase 6 (begins) — Detector library breadth.** Target total: 30 detectors. Phase 6-lite (landed 2026-04-22) took us from 6 to 12 by adding s3_public_access_block, rds_encryption_at_rest, kms_key_rotation, cloudtrail_log_file_validation, vpc_flow_logs_enabled, and iam_password_policy. Remaining work toward the 30 target covers AWS controls that appear in every real SaaS FedRAMP Moderate package: security groups, VPC config detail, S3 bucket policies, IAM Access Analyzer, GuardDuty enablement, Config rules, ECR image scanning, Secrets Manager usage, etc. Runs in parallel with Phase 4.
