...
- [x] **Branch protection on `main` configured per `.github/BRANCH_PROTECTION.md`.** Applied 2026-04-26 as GitHub Ruleset id `15566618` on `efterlev/efterlev` (lowercase, post-rename). Active enforcement, empty bypass list (no admin override), 5 required status checks (`lint, type-check, test`, `DCO`, `pip-audit (Python deps)`, `bandit (Python static security analysis)`, `semgrep (security rule set)`), required signed commits + linear history. Validated end-to-end via PRs #12, #15-#20 — every PR required signed commits and the full required-check set to pass before squash-merge. (Note: applied via GitHub's newer Rulesets UI, not the older "Branch protection rules" UI; semantics are identical, surface area is broader.)
