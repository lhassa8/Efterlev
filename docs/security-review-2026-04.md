...
| **T3** — Hallucinated evidence accepted as real | Mitigated (defense-in-depth at two layers, with structural strengthening at the per-agent layer) | (a) Per-agent fence validators reject fabricated citations: `gap.py:250 _validate_cited_ids`, `documentation.py:268 _validate_cited_ids`, `remediation.py:206 _validate_citations` (different name, same contract — naming inconsistency tracked in §7). PR #23 (2026-04-27) strengthened layer (a) with two-sided citation discipline: in addition to fabrication rejection, `KsiClassification._positive_status_requires_evidence` rejects `status="implemented"` with empty `evidence_ids` on the Gap side, and `documentation._validate_cited_ids` rejects empty `cited_evidence_ids` whenever the Gap classification cited evidence — closing the silent-decitation hole that would otherwise let a confident narrative land in the persisted Claim with empty `derived_from`. (b) Store-level: `ProvenanceStore.write_record` calls `_validate_claim_derived_from` for `record_type="claim"` records with non-empty `derived_from` BEFORE INSERT (`src/efterlev/provenance/store.py:130`). Atomicity: insert happens under `_write_lock` after the check; rejected claim never mutates the store. Tests in `tests/test_validate_claim_provenance.py` + `tests/test_documentation_agent.py` exercise the layer-(a) strengthening; both record-id and evidence-id resolution + insertion-atomicity remain covered at layer (b). |
...
- [x] **Per-agent `_validate_cited_ids` helpers** — confirmed for all three generative agents. Each rejects model-output citing IDs that didn't appear inside a per-run-nonced fence in the prompt.
  - `src/efterlev/agents/gap.py:250` — `_validate_cited_ids`
  - `src/efterlev/agents/documentation.py:268` — `_validate_cited_ids` (extended 2026-04-27 in PR #23 to also reject empty `cited_evidence_ids` when the Gap classification cited evidence — closes the silent-decitation hole)
...
- [x] **No internal hostnames, SSH keys, or credentials in any committed file.** Two layers of evidence:
  ...
  - **Full git history:** `gitleaks git --redact --config .gitleaks.toml` scans 145 commits and reports no leaks. Raw scan (without allowlist) finds 24 hits, ALL in `tests/test_scrubber.py`, `tests/test_redaction_integration.py`, and `scripts/e2e_smoke.py` — synthetic secret-shaped strings used to TEST the scrubber library. The `.gitleaks.toml` allowlist explicitly permits these test files with rationale.
...
- [x] **Branch protection on `main` configured per `.github/BRANCH_PROTECTION.md`.** Applied 2026-04-26 as GitHub Ruleset id `15566618` on `efterlev/efterlev` (lowercase, post-rename). Active enforcement, empty bypass list (no admin override), 5 required status checks (`lint, type-check, test`, `DCO`, `pip-audit (Python deps)`, `bandit (Python static security analysis)`, `semgrep (security rule set)`), required signed commits + linear history. Validated end-to-end via PRs #12, #15-#24 — every PR required signed commits and the full required-check set to pass before squash-merge. (Note: applied via GitHub's newer Rulesets UI, not the older "Branch protection rules" UI; semantics are identical, surface area is broader.)
