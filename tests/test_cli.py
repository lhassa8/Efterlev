...
def test_detectors_list_lists_all_thirty_detectors(tmp_path: pytest.TempPathFactory) -> None:
    ...
    assert result.exit_code == 0
    # 36 detectors per the A4 + Priority 1.x catalog catalog (see tests/test_smoke.py:
    # test_every_detector_folder_registers).
    assert "total: 36 detectors" in result.output
    ...
    assert "29 KSI-mapped" in result.output
