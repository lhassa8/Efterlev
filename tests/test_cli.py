...
def test_detectors_list_lists_all_thirty_detectors(tmp_path: pytest.TempPathFactory) -> None:
    ...
    assert result.exit_code == 0
    # 35 detectors per the A4 + Priority 1.x catalog catalog (see tests/test_smoke.py:
    # test_every_detector_folder_registers).
    assert "total: 35 detectors" in result.output
    ...
    assert "28 KSI-mapped" in result.output
